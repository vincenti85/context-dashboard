...
### 9.3 전제 조건: 채널이 있어야 한다

수집 쿼리는 `channel==MINE`으로 **인증한 계정이 소유한 채널**을 조회한다.
채널을 만든 적이 없는 계정으로 인증하면 설정이 모두 올바르더라도 403이 난다
(2026-07-22 설정 중 실제로 겪은 사례). 채널 개설 후 재검증하면 된다.

설정이 실제로 동작하는지는 아래로 확인한다 (대시보드 로그인 상태 필요):

```
{APP_URL}/api/youtube/oauth?verify=1
```

`"ok": true`면 토큰·스코프·API 접근이 모두 정상이다. 게시한 영상이 없어
조회수가 0으로 나오는 것은 정상이며 인증 성공과 무관하다.
